...
export class FormPosiblesVentasComponent implements OnInit {
  ...
  suma() {
    ...
    for (let row of this.listCarrito) {
      ...
      //console.log("********", row );
      //EDU se ajusta por manera definida por admin
    ...
    //EDU se comenta- por que el procesos debe ser de la manera definida por admin
  ...
  async submit() {
    ...
    this._ventas.create( data ).subscribe( async (res: any) => {
      ...
      let url = `https://wa.me/57${ this.proveedorContacto }?text=${encodeURIComponent(`
          ...
          ${ this.productoFoto }
          A tu usuario de Multitienda.com agradezco la prioridad de despacho ya que es de gran importancia para satisfacer a nuestro cliente
